...
inline void CreateManipulator(const Matrix<3>& state, const int& cal_link1, const int& cal_link2, const int& cal_link3, bool flag)
{
	...
	//int cal_joint;
	//CAL_CreateGroup(&cal_joint, 0, false, "joint");
	//CAL_SetGroupColor(cal_joint, 0, 1, 0);
	//CAL_CreateSphere(cal_joint, w/2.0, 0.0, 0.0, 0.0);
	//CAL_CreateSphere(cal_joint, w/2.0, p1[0]*2, p1[1]*2, 0.0);
	//CAL_CreateSphere(cal_joint, w/2.0, (p2[0] - p1[0])*2, (p2[1] - p1[1])*2, 0.0);

}

inline void VisualManipulator(const Matrix<3>& state, int group_id1, int group_id2, int group_id3)
{
	int obj1 = -1, obj2 = -1, obj3 = -1;
	double len = 1.5;
	double w = 0.3;
	//dynamics
	Matrix<2> p1 = zeros<2,1>();
	p1[0] = 0.5*len*cos(state[0]);
	p1[1] = 0.5*len*sin(state[0]);
	CAL_CreateBox(group_id1, len, w, 0.02, 0.0, 0.0, 0.0, &obj1);
	CAL_SetObjectPosition(obj1, (float)p1[0], (float)p1[1], 0.0);
	CAL_SetObjectOrientation(obj1, 0.0, 0.0, (float)state[0]);
	
	Matrix<2> p2 = zeros<2,1>();
	p2[0] = len*cos(state[0]) + 0.5*len*cos(state[1]);
	p2[1] = len*sin(state[0]) + 0.5*len*sin(state[1]);
	CAL_CreateBox(group_id2, len, w, 0.02, 0.0, 0.0, 0.0, &obj2);
	CAL_SetObjectPosition(obj2, (float)p2[0], (float)p2[1], 0.0);
	CAL_SetObjectOrientation(obj2, 0.0, 0.0, (float)state[1]);
	
	Matrix<2> p3 = zeros<2,1>();
	p3[0] = len*(cos(state[0]) + cos(state[1]) + 0.5* cos(state[2]));
	p3[1] = len*(sin(state[0]) + sin(state[1]) + 0.5* sin(state[2]));
	CAL_CreateBox(group_id3, len, w, 0.02, 0.0, 0.0, 0.0, &obj3);
	CAL_SetObjectPosition(obj3, (float)p3[0], (float)p3[1], 0.0);
	CAL_SetObjectOrientation(obj3, 0.0, 0.0, (float)state[2]);
...
inline void VisualManipulator1(const Matrix<3>& state)
{
	double len = 1.5;
	double w = 0.3;
	//dynamics

	int cal_l1, cal_l2, cal_l3;
	CAL_CreateGroup(&cal_l1, 0, false, "l1");
	CAL_SetGroupColor(cal_l1, 1, 0, 0);
	CAL_CreateGroup(&cal_l2, 0, false, "l2");
	CAL_SetGroupColor(cal_l2, 1, 0, 0);
	CAL_CreateGroup(&cal_l3, 0, false, "l3");
	CAL_SetGroupColor(cal_l3, 1, 0, 0);

	Matrix<2> p1 = zeros<2,1>();
	p1[0] = 0.5*len*cos(state[0]);
	p1[1] = 0.5*len*sin(state[0]);
	CAL_CreateBox(cal_l1, len, w, 0.02, 0.0, 0.0, 0.0);
	CAL_SetGroupPosition(cal_l1, p1[0], p1[1], 0.0);
	CAL_SetGroupOrientation(cal_l1, 0.0, 0.0, state[0]);

	Matrix<2> p2 = zeros<2,1>();
	p2[0] = len*cos(state[0]) + 0.5*len*cos(state[1]);
	p2[1] = len*sin(state[0]) + 0.5*len*sin(state[1]);
	CAL_CreateBox(cal_l2, len, w, 0.02, 0.0, 0.0, 0.0);
	CAL_SetGroupPosition(cal_l2, p2[0], p2[1], 0.0);
	CAL_SetGroupOrientation(cal_l2, 0.0, 0.0, state[1]);

	Matrix<2> p3 = zeros<2,1>();
	p3[0] = len*(cos(state[0]) + cos(state[1]) + 0.5* cos(state[2]));
	p3[1] = len*(sin(state[0]) + sin(state[1]) + 0.5* sin(state[2]));
	CAL_CreateBox(cal_l3, len, w, 0.02, 0.0, 0.0, 0.0);
	CAL_SetGroupPosition(cal_l3, p3[0], p3[1], 0.0);
	CAL_SetGroupOrientation(cal_l3, 0.0, 0.0, state[2]);
...
inline Matrix<2> forward_k(const Matrix<3,1>& state, const int& K, const double& l)
{
	double len = 1.5;
	double w = 0.3;
	...
	{
		Matrix<2> p = zeros<2,1>();
		p[0] = len*cos(state[0]) + l*cos(state[1]);
		p[1] = len*sin(state[0]) + l*sin(state[1]);
		return p;
	}
	...
	{
		Matrix<2> p = zeros<2,1>();
		p[0] = len*(cos(state[0]) + cos(state[1])) + l*cos(state[2]);
		p[1] = len*(sin(state[0]) + sin(state[1])) + l*sin(state[2]);
		return p;
	}
...
inline bool lineCollision(const Matrix<3>& state1, const Matrix<3>& state2, const int& cal_link1, const int& cal_link2, const int& cal_link3, const int& cal_obstacles)
{
	for(int i = 0; i <= 10; i++){
		double alpha = i * 1.0 / 10;
		Matrix<3> tmpp = alpha * state1 + (1 - alpha)*state2;
		bool sucess = pointCollision(tmpp, cal_link1, cal_link2, cal_link3, cal_obstacles);
		if(sucess == false)
			return false;
	}
	return true;
